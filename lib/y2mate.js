...
async function yt(url, quality, type, bitrate, server = 'en68') {
  ...
  let KB = parseFloat(filesize) * (10 * /MB$/.test(filesize))
...
module.exports = {
   ...
   * @param {String} server (avaiable: `id4`, `en60`, `en61`, `en68`)
   */
  ytv(url, resol = '240p', server = 'en154') { return yt(url, resol, 'mp4', resol.endsWith('p') ? resol.replace(/p/g, '') : resol, server) },
    ytv2(url, resol = '144p', server = 'en154') { return yt(url, resol, 'mp4', resol.endsWith('p') ? resol.replace(/p/g, '') : resol, server) },
